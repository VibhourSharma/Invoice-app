...
const InvoiceList = ({ filteredData, statusColors }) => (
  <>
    {filteredData.map((invoice) => (
      <Link to={`/InvoiceDashboard/receipt/${invoice.id}`} key={invoice.id}>
        <div className="flex rounded-lg h-24 bg-white mt-4 justify-between dark:bg-[#1E2139] hover:border hover:border-[#7C5DFA] cursor-pointer">
          <div className="flex items-center">
            ...
            <span className="text-slate-500 mr-12">{invoice.createdAt}</span>
            <span className="text-slate-500">{invoice.clientName}</span>
          </div>
          <div className="flex items-center">
            <span className="font-bold text-xl mr-12">£ {invoice.total}</span>
            <div
